...
describe('blockwise2', function () {
    ...
    it('should server not use blockwise in response when payload fit in one packet', function (done) {
        ...
        (0, index_1.request)({
            ...
            (0, chai_1.expect)(blockwiseResponse).to.eql(false);
    ...
    it('should use blockwise in response when payload bigger than max packet', function (done) {
        (0, index_1.request)({
            ...
            (0, chai_1.expect)(blockwiseResponse).to.eql(true);
    ...
    it('should blockwise response have etag', function (done) {
        (0, index_1.request)({
            ...
            .on('response', (res) => {
            (0, chai_1.expect)(typeof res.headers.ETag).to.eql('string');
            // expect(cache.get(res._packet.token.toString())).to.not.be.undefined
            setImmediate(done);
        })
            .end();
    ...
    it('should accept early negotation', function (done) {
        (0, index_1.request)({
            ...
            (0, chai_1.expect)(block2 instanceof Buffer).to.eql(true);
            (0, chai_1.expect)(block2[block2.length - 1] & 0x07).to.eql(2);
            // expect(cache.get(res._packet.token.toString())).to.not.be.undefined
            setImmediate(done);
        })
            .end();
    ...
    it('should receive error when early negotation request block size higher than 1024', function (done) {
        (0, index_1.request)({
            ...
            .on('response', (res) => {
            (0, chai_1.expect)(res.code).to.eql('4.02');
            // expect(cache.get(res._packet.token.toString())).to.be.undefined
            setImmediate(done);
        })
            .end();
    ...
    it('should be able to receive part of message', function (done) {
        (0, index_1.request)({
            ...
            .on('response', (res) => {
            (0, chai_1.expect)(res.payload).to.eql(payload.slice(1 * 16, payload.length + 1));
            // expect(cache.get(res._packet.token.toString())).to.not.be.undefined
            setImmediate(done);
        })
            .end();
    ...
    it('should receive full response payload', function (done) {
        ...
        (0, index_1.request)({
            ...
            .on('response', (res) => {
            (0, chai_1.expect)(res.payload).to.eql(payload);
            // expect(cache.get(res._packet.token.toString())).to.not.be.undefined
            setImmediate(done);
        })
            .end();
    ...
    function parallelBlock2Test(done, checkNReq, checkBlock2Message, checkNormalReq) {
        ...
        let req2Done = false;
        let req1Block2Num = 0;
        const reqClient2 = (0, index_1.request)({
            port
        });
    ...
    it('should two parallel block2 requests should result only two requests to upper level', function (done) {
        const checkNreq = (nreq) => {
            (0, chai_1.expect)(nreq).to.be.within(1, 2);
        };
    ...
    it('should have correct block2 option for parallel requests', function (done) {
        const checkBlock2Option = (msg, payloadReq1, req1Block2Num, payloadLength) => {
            const res = (0, coap_packet_1.parse)(msg);
            // Have block2 option?
            const block2Buff = (0, helpers_1.getOption)(res.options, 'Block2');
            if (block2Buff instanceof Buffer) {
                const block2 = (0, helpers_1.parseBlock2)(block2Buff);
                (0, chai_1.expect)(block2).to.not.eql(null);
                const expectMore = (req1Block2Num + 1) * 16 <= payloadLength ? 1 : 0;
                // Have correct num / moreBlocks fields?
                if (block2 != null) {
                    (0, chai_1.expect)(block2.num).to.eql(req1Block2Num);
                    (0, chai_1.expect)(block2.more).to.eql(expectMore);
                }
                else {
    ...
    it('should have correct payload in block2 messages for parallel requests', function (done) {
        const checkBlock2Payload = (msg, payloadReq1, req1Block2Num) => {
            ...
            (0, chai_1.expect)(res.payload).to.eql(payloadReq1.slice(req1Block2Num * 16, req1Block2Num * 16 + 16));
        };
        const checkNormalRespPayload = (res, payloadReq2) => {
            // Have correct payload?
            (0, chai_1.expect)(res.payload).to.eql(payloadReq2.slice(1 * 16, payload.length + 1));
        };
    ...
    it('should support the Size2 option', function (done) {
        (0, index_1.request)({
            ...
            (0, chai_1.expect)(size2).to.eql(payload.length);
            setImmediate(done);
        })
            .end();
        server.on('request', (req, res) => {
...
describe('blockwise1', () => {
    ...
    describe('Parse Block Options', () => {
        it('it should return object', (done) => {
            ...
            const value = (0, block_1.parseBlockOption)(payload);
            (0, chai_1.expect)(value).to.eql(response);
            setImmediate(done);
        });
        ...
        it('it should throw Invalid Block Option error', (done) => {
            ...
            (0, chai_1.expect)(() => {
                (0, block_1.parseBlockOption)(payload);
    ...
    describe('Exponenent to Byte Size', () => {
        it('it should return value', (done) => {
            const response = 1024;
            const payload = 6;
            const value = (0, block_1.exponentToByteSize)(payload);
            (0, chai_1.expect)(value).to.eql(response);
            setImmediate(done);
        });
    });
    describe('Byte Size to Exponenet', () => {
        it('it should return value', (done) => {
            const response = 1024;
            const payload = 6;
            const value = (0, block_1.byteSizeToExponent)(response);
            (0, chai_1.expect)(value).to.eql(payload);
            setImmediate(done);
        });
    });
});
//# sourceMappingURL=blockwise.js.map