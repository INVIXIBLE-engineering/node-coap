...
describe('request', function () {
    ...
    it('should be able to include accept and Content-Format parameters', function (done) {
        ...
        server.on('message', (msg) => {
            const packet = (0, coap_packet_1.parse)(msg);
            (0, chai_1.expect)(packet.options[0].name).to.eql('Content-Format');
            (0, chai_1.expect)(packet.options[0].value).to.eql(Buffer.of(0x01, 0xb0));
            (0, chai_1.expect)(packet.options[1].name).to.eql('Accept');
            (0, chai_1.expect)(packet.options[1].value).to.eql(Buffer.of(60));
            done();
        });
        req.end();
    });
    ...
    describe('observe', function () {
        ...
        it('should send an empty Observe option', function (done) {
            ...
            server.on('message', (msg, rsinfo) => {
                const packet = (0, coap_packet_1.parse)(msg);
                (0, chai_1.expect)(packet.options[0].name).to.eql('Observe');
                (0, chai_1.expect)(packet.options[0].value).to.eql(Buffer.alloc(0));
                done();
            });
        });
